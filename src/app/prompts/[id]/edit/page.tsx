  ...
  const fetchPrompt = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Parse sections
        let parsedSections: PromptSection[] = []
        if (data.sections) {
          try {
        ...
        if (parsedSections.length === 0) {
          if (data.instructions) {
            parsedSections.push({ id: '1', type: 'instruction', content: data.instructions })
          }
          if (data.content) {
            parsedSections.push({ id: '2', type: 'prompt', content: data.content })
          }
        }
        setSections(parsedSections)

        // Convert sample outputs
        const convertedOutputs = data.sampleOutputs.map((output: any) => ({
          id: output.id,
          title: output.title || '',
          ...
          attachedFiles: output.filePath ? [{
            filename: output.filePath.split('/').pop(),
            originalName: output.filePath.split('/').pop(),
            filePath: output.filePath,
            fileType: output.fileType || '',
            size: 0,
            category: output.outputType === 'IMAGE' ? 'images' : 'documents'
          }] : []
        }))
        setSampleOutputs(convertedOutputs)
      }
    } catch (error) {
